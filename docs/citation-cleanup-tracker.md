...
| A Greater Town | Listing appears in search results, but source-specific searches still did not show an old phone snippet. | Unconfirmed | Check direct listing before requesting correction. | Listing contact/correction path. | needs_access | Rechecked 2026-06-20: direct listing `https://agreatertown.com/cleveland-oh/longpro-pest-control-0001636885848`, homepage, `/contact`, `/about`, `/privacy`, and `sitemap.xml` all challenge this repo-side environment with Cloudflare 403s. `robots.txt` is reachable, but the listing content is not. Needs normal-browser direct verification before any correction request. |
...
1. Owner side: handle Contractors Up first, then Manta, A Greater Town, Yelp/YellowPages browser verification, and Google Business Profile completion.
2. Repo side: rescan citations after provider processing windows or owner-side fixes and keep `docs/citation-evidence/latest.json` plus this tracker current.
